...
type JobPosition = {
  ...
  typeVi: string; typeEn: string;
  locationVi: string; locationEn: string;
...
export default function CareersPageContent() {
  ...
  // Danh sách vị trí do admin quản lý tại /admin/careers.
  const [positions, setPositions] = useState<JobPosition[]>([]);
  useEffect(() => {
    fetch('/api/job-positions')
      .then(r => (r.ok ? r.json() : []))
      .then((data: JobPosition[]) => setPositions(data))
      .catch(() => {});
  }, []);
