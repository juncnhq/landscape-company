...
export default function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Hiện đúng message server trả về. Trước đây luôn ghi cứng "sai mật
      // khẩu", nên khi bị chặn vì đăng nhập sai quá nhiều lần (429) người
      // dùng vẫn tưởng mình gõ nhầm và cứ thử lại.
      let msg = 'Email hoặc mật khẩu không đúng.'
      try {
        const data = await res.json()
        if (data && typeof data.error === 'string' && data.error.trim()) msg = data.error
      } catch {
        // body rỗng hoặc không phải JSON — giữ message mặc định
      }
      setError(msg)
    } catch {
      // Không bắt lỗi ở đây thì `fetch` đứt mạng sẽ reject, setLoading(false)
      // không bao giờ chạy và nút kẹt "Đang xử lý..." vĩnh viễn.
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                required
                autoFocus
                className="w-full px-4 py-2.5 rounded-md border border-gray-200 text-sm text-gray-900 bg-white focus:outline-none focus:ring-2 focus:ring-[#328442]/30 focus:border-[#328442] transition-colors"
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-[#328442] hover:bg-[#48a85a] text-white py-2.5 rounded-md text-sm font-medium transition-colors disabled:opacity-50 mt-2"
            >
